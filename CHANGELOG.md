...
- **While-loop Pentagon bounds** (`Intervals.fs`, `StmtFuncAnalysis.fs`): `extractPentagonBoundsFromCondition` parses while-loop conditions for relational bounds; handles `<=`, `<`, `>=`, `>` comparisons and `&&` conjunctions; the while-loop handler now saves and restores `upperBounds` and `lowerBounds` alongside `valueRanges` (previously only `valueRanges` was restored)
- **2 new switch/case tests** in `tests/intervals/`: `switch_multi_value_refine.m` (`case {1, 5}` narrows `n` to hull `[1, 5]`; `zeros(n, n)` inside the arm sees the bounded size), `switch_cell_single.m` (single-element cell case `case {3}` narrows to `[3, 3]`)
- **3 new Pentagon lower-bound tests** in `tests/intervals/`: `pentagon_lower_bound.m` (for-loop with variable start, lower bridge suppresses "index may be < 1"), `pentagon_lower_concrete.m` (concrete lower bound `start = 1`, bridge fires with exact value), `pentagon_lower_branch.m` (lower-bound refines inside an if-branch condition)
- **2 new while-loop Pentagon tests** in `tests/intervals/`: `while_pentagon_upper.m` (`while i <= n` records `i <= n` and suppresses OOB on `A(i, 1)`), `while_pentagon_compound.m` (`while i <= n && j >= 1` exercises `&&` conjunction parsing in `extractPentagonBoundsFromCondition`)
- **3 new workspace_adv tests**: `MyVehicle.m` (classdef file declaring `speed` and `capacity` properties), `ws_classdef_ctor.m` (constructs `MyVehicle` cross-file; result is `struct{speed: unknown, capacity: unknown}`), `ws_classdef_method.m` (calls a method on a cross-file classdef object)
- Total test count: 431 (was 421); `intervals/` grew by 7 (switch multi-value, pentagon lower bounds, while pentagon); `workspace_adv/` grew by 3 (cross-file classdef)
